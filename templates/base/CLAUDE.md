...
Harness `{{HARNESS_VERSION}}`. Las skills viven **en este repo**, en `.claude/skills/`, y
...
- **P2 — el dominio jamás importa frameworks.** `adapters → application → domain`, nunca
  al revés. Un framework nunca es un puerto. El enforcement ({{ARCH_ENFORCER}}) corre en
...
español rioplatense/argentino. Aplica a **toda** salida: la sesión principal y **todos los
