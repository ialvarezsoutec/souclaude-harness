...
**Debe quedar:** milestone `{{PREFIJO}}-M1` En curso con plan `P1`, su rama `feature/{{PREFIJO}}-M1-<slug>` creada desde `dev` (una rama por milestone; las tareas son commits en ella) y tareas `{{PREFIJO}}-M1-T00n`; los issues correspondientes en Jira con la etiqueta del milestone; y la línea de sesión registrada.
